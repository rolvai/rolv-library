ROLV Official Hardware Verification
& Benchmarking Kit
...
2. The Role of SHA-256 Hashes
To prove our results are legitimate, we use SHA-256 hashing as a mathematical proof of equivalence:
...
Note: In rare cases, hashes may vary slightly across CUDA versions or hardware due to floating-point precision. We confirm equivalence numerically (e.g., atol=1e-5) to ensure legitimacy.
...
NVIDIA GPUs	pynvml polls the GPU power rail every 50 ms; joules are computed via trapezoidal integration of real samples.
AMD GPUs	pyrsmi provides equivalent live readings where the driver supports it.
CPU / Apple Silicon	Energy is estimated from psutil CPU utilization × TDP — clearly labelled as an estimate in the output JSON.
Reporting	The energy_measurement_method field in the output JSON always records which method was used.

4. Python Verification Script
Save the code below as rolv-verifier.py or paste it into a Jupyter Notebook cell. Both square and rectangular matrix shapes are fully supported — set TEST_ROWS and TEST_COLS independently to match your workload dimensions.
...
5. Reproducibility & System Requirements
To ensure consistent results and hash matches, we recommend the following setup:

•	PyTorch Version: Use PyTorch 2.5.0 or later with CUDA support (e.g., torch==2.5.1+cu121). Install via pip install torch==2.5.1 --index-url https://download.pytorch.org/whl/cu121
•	CUDA Version: CUDA 12.1 is ideal for exact reproducibility. Check your version with nvcc --version or torch.version.cuda. If using a different version (e.g., 12.8), minor floating-point differences may occur due to library optimizations — contact us if hashes mismatch.
•	Determinism Tips: The script includes seeding for reproducibility. If running on GPU, add these lines before the benchmark code for extra stability:

torch.backends.cudnn.deterministic = True
torch.backends.cudnn.benchmark = False
os.environ['CUBLAS_WORKSPACE_CONFIG'] = ':4096:8'

•	If Hashes Differ: This can happen due to hardware (e.g., V100 vs. H100) or version differences. We verify numerically (close floating-point match) on our end. If you encounter issues, email us your setup details.
...
4.	Download Anaconda (which includes Jupyter Notebook) from anaconda.com/download
5.	Open a new Jupyter Notebook.
6.	Paste the code above into a cell and press Shift + Enter
7.	Enter your email when prompted.
8.	Submission: Email the .json file created in your folder, or copy the text between [ GENERATED DATA PACKAGE ] and the final } and paste it into an email to rolv@rolv.ai
Important Notes
•	The output file will be named rolv_baseline_<your_email>_<timestamp>.json.
•	This script does not contain ROLV — it only captures your hardware baseline. All ROLV computation and comparison happens on our infrastructure after you submit.
•	If the script warns about NVML or hash issues, rerun after checking your PyTorch/CUDA versions match our recommendations above.
